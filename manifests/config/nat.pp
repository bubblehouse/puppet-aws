class bootstrap::config::nat {
  $iface = "eth1"
  $mac = inline_template("<%= scope.lookupvar(['$macaddress_' + scope.lookupvar(['$iface'])]) %>")
  $range = inline_template("<%= scope.lookupvar(['$ec2_network_interfaces_macs_' + scope.lookupvar(['$mac']) + '_vpc_ipv4_cidr_block']) %>")
